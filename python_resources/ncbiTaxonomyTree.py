...
class NcbiTaxonomyTree(object):
    ...
    def __init__(self, nodes_filename=None, names_filename=None):
        ...
        if nodes_filename and names_filename:
            ...
            self.dic = {}  # Maps taxid to Node named tuple
            self.taxid_to_ptaxid = {}  # For testing purposes
            self.ptaxid_to_taxid = {}  # For testing purposes
            with open(nodes_filename) as nodes_file:
                for line in nodes_file:
                    ...
                    # LCAStar compatibility
                    self.taxid_to_ptaxid[taxid] = [parent_taxid, 0, 0]
                    if parent_taxid not in self.ptaxid_to_taxid:
                        self.ptaxid_to_taxid[parent_taxid] = {}
                    if not (parent_taxid == 1 and taxid == 1):
                        if taxid == parent_taxid:
                            logging.error("What did you just do!. You should have never gotten here!" + "\n")
                            sys.exit(0)
                    self.ptaxid_to_taxid[parent_taxid][taxid] = False

                    if taxid in self.dic:  # 18204/1308852
                        self.dic[taxid] = self.dic[taxid]._replace(rank=line[2][1:-1],
                                                                   parent=parent_taxid)
                    ...
                    try:  # 1290648/1308852
                        ...
                        self.dic[parent_taxid] = Node(name=taxid2name[parent_taxid],
                                                      rank=None,
                                                      parent=None,
                                                      children=[taxid])
    ...
    def getParent(self, taxids):
        """
            >>> NcbiTaxonomyTree(nodes_filename="nodes.dmp", names_filename="names.dmp").getParent([28384, 131567])
            {28384: 1, 131567: 1}
        """
        result = {}
        for taxid in taxids:
